...
def Test(model: LightGCN, batch_users_gpu: Tensor, origin_train_data: DataSet, test_user_item_map: Dict[int, Set[int]]) -> Tuple[float, float, float, float]:
    ...
    rating[exclude_index, exclude_items] = -1  # 因为sigmoid输出是(0,1)
    _, topk_indices = torch.topk(rating, k=config.topk)
    topk_indices = topk_indices.tolist()  # 降序排列的top-k个待推荐物品矩阵（未去除用户交互过的物品）
    for user_id, items in test_user_item_map.items():
        rec_list = topk_indices[user_id]  # 模型的推荐列表
